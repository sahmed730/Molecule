...
function Dashboard() {
  ...
  const handleClarifyLoop = async (skip: boolean = false) => {
    if (!prompt.trim()) return;

    // If skip is true or we are just starting, build structured answers
    const newStructured: Record<string, string> = { ...accumulatedAnswers };
    if (!skip && questions.length > 0) {
      questions.forEach(q => {
        const ans = answers[q.id];
        const label = q.question;
        if (Array.isArray(ans)) {
          newStructured[label] = ans.length > 0 ? ans.join(', ') : 'Not specified';
        } else {
          newStructured[label] = ans || 'Not specified';
        }
      });
      setAccumulatedAnswers(newStructured);
    }

    if (skip) {
       return handleGenerateArchitecture(undefined, newStructured);
    }
...
` 
    ...
    try {
      const clarifyRes = await fetch(`${API_BASE}/api/clarify-architecture`, {
        ...
        body: JSON.stringify({ 
           ...
           answers: Object.keys(newStructured).length > 0 ? newStructured : undefined
        }),
      });
      const clarifyData = await clarifyRes.json();

      if (clarifyData.status === 'success') {
         if (clarifyData.data.confidence_high || !clarifyData.data.questions || clarifyData.data.questions.length === 0) {
            // High confidence or no questions -> proceed to generation
            handleGenerateArchitecture(undefined, newStructured);
         } else {
            // Show new questions
      ...
      console.error('Pipeline loop failed:', e);
      handleGenerateArchitecture(undefined, newStructured);
    } finally {
  ...
  const handleGenerateArchitecture = async (
    ...
    const finalAnswers = providedAnswers || accumulatedAnswers;
        ...
        const newNodes = rawModules.map((mod: any, index: number) => {
          ...
          return {
            ...
            data: {
              ...
              communicationContracts: mod.communicationContracts || '',
              technologyStack: mod.technologyStack || '',
              dependencies: mod.dependencies || '',
              constraints: mod.constraints || '',
              nonFunctional: mod.nonFunctional || '',
              testing: mod.testing || '',
              deployment: mod.deployment || '',
              architectureDecisions: mod.architectureDecisions || '',
              status: 'ready',
            },
          };
        });
  ...
  return (
    <div className="flex w-full h-screen bg-neutral-primary text-body overflow-hidden">
      ...
      <header className="fixed top-6 left-1/2 -translate-x-1/2 w-full max-w-[1280px] h-16 bg-neutral-primary-soft/80 backdrop-blur-[16px] border border-default rounded-[12px] flex items-center justify-between px-6 z-50 shadow-md">
        <div className="flex items-center cursor-pointer group h-full py-3" onClick={() => setAppState('home')}>
          <img src="/logo-full.png" alt="EasePr Full Logo" className="h-full object-contain transform group-hover:scale-105 transition-transform" />
        </div>
